...
void EventHandler::onCreateEventSubmitted(QString eventName, QStringList dates, QTime startTime, QTime endTime)
{
    ...
    for (const QString &dateStr : dates) {
        QDate date = QDate::fromString(dateStr, "yyyy-MM-dd"); // Adjust format if your UI uses a different one
        if (date.isValid()) {
    ...
    if (!m_event) {
        m_event = new Event(); // Assuming m_event is an Event* pointer managed by EventHandler
    }
    m_event->setTitle(eventName);
    m_event->setSelectedDates(convertedDates);
    m_event->setTimeRange(startTime, endTime);

    // 3. Generate the invite code and notify the MainWindow
